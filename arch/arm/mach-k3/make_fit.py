...
def get_overlays(board):
    ...
    """
    overlays = []
    var = get_u_boot_var("CONFIG_OF_OVERLAY_LIST")
    if not var:
	    return []

    for ov in var.strip('"').split():
        if os.path.isfile("arch/arm/dts/{}.dtbo".format(ov)):
            overlays.append("{}.dtbo".format(ov))
	else:
            sys.stderr.write("cannot find {}.dtbo. removing from its!\n".format(ov))
    return overlays
...
def generate_its(dtbs):
    sys_text_base = get_u_boot_var("CONFIG_SYS_TEXT_BASE")
...
def usage():
    print(
        "{} <dt_name> [<dt_name> [<dt_name] ...] ".format(
            sys.argv[0]))
    sys.exit(-1)
