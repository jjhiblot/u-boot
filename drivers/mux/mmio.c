 ...
 * (Heavily based on the linux mmio mux driver)
 *
...
static int mmio_mux_probe(struct udevice *dev)
{
	...
	mux_reg_masks = devm_kmalloc(dev, num_fields * 2 * sizeof(u32), __GFP_ZERO);
	if (!mux_reg_masks)
		return -ENOMEM;

	printf("mux_reg_masks %p\n", mux_reg_masks);
	
	ret = dev_read_u32_array(dev, "mux-reg-masks", mux_reg_masks, num_fields * 2);
	if (ret < 0) {
	...
	for (i = 0; i < num_fields; i++) {
		...
		if (idle_states) {
			if (idle_states[i] != MUX_IDLE_AS_IS &&  idle_states[i] >= mux->states) {
