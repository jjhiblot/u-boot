				   ...
				   int index, struct mux_control *mux)
{
	...
	ret = dev_read_phandle_with_args(dev, prop_name, "#mux-control-cells", 0,
					 index, &args);
	if (ret) {
...
struct mux_control *devm_mux_control_get(struct udevice *dev, const char *id)
{
	...
	mux = devres_alloc(devm_mux_control_release, sizeof(struct mux_control), __GFP_ZERO);
	if (unlikely(!mux))
