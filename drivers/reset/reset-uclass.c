...
static int __reset_get_bulk(struct udevice *dev, ofnode node, struct reset_ctl_bulk *bulk)
{
	int i, ret, err, count;
						  ...
						  int index)
{
	...
	reset_ctl = devres_alloc(devm_reset_release, sizeof(struct reset_ctl), __GFP_ZERO);
...
struct reset_ctl *devm_reset_control_get_optional(struct udevice *dev, const char *id)
{
...
struct reset_ctl_bulk *devm_reset_bulk_get_by_node(struct udevice *dev, ofnode node)
{
	int rc;
	struct reset_ctl_bulk *bulk;

	bulk = devres_alloc(devm_reset_bulk_release, sizeof(struct reset_ctl_bulk), __GFP_ZERO);
	if (unlikely(!bulk))
		return ERR_PTR(-ENOMEM);
...
struct reset_ctl_bulk *devm_reset_bulk_get_optional_by_node(struct udevice *dev, ofnode node)
{
	struct reset_ctl_bulk *bulk;
