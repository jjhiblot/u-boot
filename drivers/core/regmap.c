			     ...
			     const struct regmap_config *config)
{
	...
	mapp = devres_alloc(devm_reset_release, sizeof(struct regmap *), __GFP_ZERO);
	if (unlikely(!mapp))
			   ...
			   const void *val, size_t val_len)
{
	...
	if (map->reg_write)
		return map->reg_write(map->bus_context, offset, *(unsigned int*)val);
	if (range_num >= map->range_count) {
		debug("%s: range index %d larger than range count\n",
		      __func__, range_num);
		return -ERANGE;
...
{
	return regmap_update_bits(field->regmap, field->reg, field->mask, val << field->shift);
}
...
void devm_regmap_field_free(struct udevice *dev,
